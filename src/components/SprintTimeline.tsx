  ...
  return (
    <div className='w-full max-w-3xl mx-auto'>
      <div className='space-y-8'>
        {sprints.map((sprint) => (
          <div key={sprint.number} className='relative'>
            {/* Track line */}
            {sprint.number < sprints.length && (
              <div className={`absolute left-7 top-12 h-full w-0.5 ${sprint.completed ? 'bg-primary' : 'bg-muted'}`} />
            )}
            {/* Sprint card */}
            <div
              className={`relative flex items-start p-6 rounded-lg border ${
                sprint.completed ? 'border-primary bg-primary/5' : 'border-muted bg-background'
              }`}
            >
              {/* Sprint status icon */}
              <div className='mr-4 flex-shrink-0 mt-1'>
                <span className='text-xl'>{sprint.completed ? '✅' : '⏳'}</span>
              </div>
              {/* Sprint details */}
              <div className='flex-grow'>
                {/* Sprint title and weeks */}
