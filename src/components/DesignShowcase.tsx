...
export function DesignShowcase() {
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {designs.map((design, index) => <div key={index} className="overflow-hidden rounded-lg border bg-background shadow-sm hover:shadow-md transition-shadow">
          <div className="aspect-video overflow-hidden">
            <img src={design.imageUrl} alt={design.title} className="h-full w-full object-cover transition-transform hover:scale-105" />
          </div>
          <div className="p-4">
